...
import { approveAssetAction, rejectAssetAction } from "@/features/approvals/actions";
import type { contentApprovals } from "@/db/schema";

type ApprovalItem = typeof contentApprovals.$inferSelect;
...
export default function ClientApprovalsGrid({
  initialApprovals,
}: {
  ...
  async function handleApprove(id: string) {
    const result = await approveAssetAction(id);
    if (!result.success) {
      toast.error(result.error ?? "Failed to approve asset");
      return;
    }
    setApprovals((prev) =>
      prev.map((a) => (a.id === id ? { ...a, status: "APPROVED", feedback: null } : a)),
    );
    setExpandedRevision(null);
    toast.success("Asset successfully verified and marked as APPROVED.");
  }

  async function handleRequestRevision(id: string) {
    const feedback = revisionInputs[id]?.trim();
    if (!feedback) return;
    const result = await rejectAssetAction(id, feedback);
    if (!result.success) {
      toast.error(result.error ?? "Failed to reject asset");
      return;
    }
    setApprovals((prev) =>
      prev.map((a) => (a.id === id ? { ...a, status: "REJECTED", feedback } : a)),
    );
    setRevisionInputs((prev) => ({ ...prev, [id]: "" }));
    setExpandedRevision(null);
    toast.success("Revision request submitted directly to the agency desk.");
  }
  ...
  const statusBadge = (status: ApprovalItem["status"]) => {
    switch (status) {
      case "APPROVED":
        return (
    ...
    <>
      ...
      {approvals.length === 0 && (
        <div className="rounded-xl border border-slate-100 bg-white p-12 text-center shadow-sm">
          <p className="text-sm text-slate-400">No approval items found.</p>
        </div>
      )}
    </>
  );
